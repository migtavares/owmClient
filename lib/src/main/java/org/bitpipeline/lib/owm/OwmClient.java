 ...
 * 
 * <strong>Note:</strong> Instances of this class are immutable
 * 
 * @author mtavares
 */
public class OwmClient {
	...
	public OwmClient(String apiKey) {
		this.owmAPPID = checkNotNull(apiKey, "apiKey");
		this.httpClient = new DefaultHttpClient();
	}

	/** Protected for testing purposes */
	protected OwmClient(String apiKey, HttpClient httpClient) {
		this.owmAPPID = checkNotNull(apiKey, "apiKey");
	...
	private <T> T checkNotNull(T fieldToCheck, String fieldName) {
		if (fieldToCheck == null) {
			throw new IllegalArgumentException(
					"Can't construct a OwmClient with a null " + fieldName);
		}
		return fieldToCheck;
	}
	 ...
	 *             a error.
	 */
	public WeatherStatusResponse currentWeatherAroundPoint(float lat,
			float lon, int cnt) throws IOException, JSONException { // , boolean
																	// cluster,
																	// OwmClient.Lang
																	// lang) {
		String subUrl = String.format(Locale.ROOT,
				"find/station?lat=%f&lon=%f&cnt=%d&cluster=yes",
	...
	public WeatherStatusResponse currentWeatherAtCity(float lat, float lon,
			int cnt) throws IOException, JSONException { // , boolean cluster,
															...
															// lang) {
		String subUrl = String.format(Locale.ROOT,
				"find/city?lat=%f&lon=%f&cnt=%d&cluster=yes",
				Float.valueOf(lat), Float.valueOf(lon), Integer.valueOf(cnt));
		JSONObject response = doQuery(subUrl);
		return new WeatherStatusResponse(response);
	}
	...
	public WeatherStatusResponse currentWeatherInBoundingBox(float northLat,
			float westLon, float southLat, float eastLon) throws IOException,
			JSONException { // , boolean cluster, OwmClient.Lang lang) {
		String subUrl = String.format(Locale.ROOT,
				"find/station?bbox=%f,%f,%f,%f&cluster=yes",
	 ...
	 *             a error.
	 */
	public WeatherStatusResponse currentWeatherAtCityBoundingBox(
			float northLat, float westLon, float southLat, float eastLon)
			throws IOException, JSONException { // , boolean cluster,
												// OwmClient.Lang lang) {
		String subUrl = String.format(Locale.ROOT,
				"find/city?bbox=%f,%f,%f,%f&cluster=yes",
				Float.valueOf(northLat), Float.valueOf(westLon),
				Float.valueOf(southLat), Float.valueOf(eastLon));
		JSONObject response = doQuery(subUrl);
		return new WeatherStatusResponse(response);
	...
	public WeatherStatusResponse currentWeatherInCircle(float lat, float lon,
			float radius) throws IOException, JSONException { // , boolean
																...
																// lang) {
		String subUrl = String.format(Locale.ROOT,
				"find/station?lat=%f&lon=%f&radius=%f&cluster=yes",
	...
	public WeatherStatusResponse currentWeatherAtCityCircle(float lat,
		...
		String subUrl = String.format(Locale.ROOT,
				"find/city?lat=%f&lon=%f&radius=%f&cluster=yes",
				Float.valueOf(lat), Float.valueOf(lon), Float.valueOf(radius));
		JSONObject response = doQuery(subUrl);
		return new WeatherStatusResponse(response);
	}
	...
	public StatusWeatherData currentWeatherAtCity(int cityId)
		...
		String subUrl = String.format(Locale.ROOT, "weather/city/%d?type=json",
	...
	public StatusWeatherData currentWeatherAtStation(int stationId)
			throws IOException, JSONException {
		String subUrl = String.format(Locale.ROOT,
				"weather/station/%d?type=json", Integer.valueOf(stationId));
		JSONObject response = doQuery(subUrl);
		return new StatusWeatherData(response);
	}
	...
	public WeatherStatusResponse currentWeatherAtCity(String cityName,
		...
		String subUrl = String.format(Locale.ROOT, "find/name?q=%s,%s",
	...
	public WeatherForecastResponse forecastWeatherAtCity(int cityId)
		...
		String subUrl = String.format(Locale.ROOT,
				"forecast/city/%d?type=json&units=metric", cityId);
		JSONObject response = doQuery(subUrl);
	...
	public WeatherForecastResponse forecastWeatherAtCity(String cityName)
			throws JSONException, IOException {
		String subUrl = String.format(Locale.ROOT,
				"forecast/city?q=%s&type=json&units=metric", cityName);
		JSONObject response = doQuery(subUrl);
		return new WeatherForecastResponse(response);
	}
	...
	public WeatherHistoryCityResponse historyWeatherAtCity(int cityId,
		...
		String subUrl = String.format(Locale.ROOT, "history/city/%d?type=%s",
				cityId, type);
		JSONObject response = doQuery(subUrl);
	...
	public WeatherHistoryStationResponse historyWeatherAtStation(int stationId,
		...
		if (type == HistoryType.UNKNOWN)
			throw new IllegalArgumentException(
					"Can't do a historic request for unknown type of history.");
		String subUrl = String.format(Locale.ROOT,
				"history/station/%d?type=%s", stationId, type);
		JSONObject response = doQuery(subUrl);
