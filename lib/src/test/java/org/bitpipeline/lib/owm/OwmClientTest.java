...
public class OwmClientTest {
	...
	private static final String TESTING_API_KEY = UUID.randomUUID().toString();

	private HttpClient createHttpClientThatRespondsWith(
			final String responseBody) throws IOException {
		HttpClient mockHttpClient = mock(HttpClient.class);
		when(mockHttpClient.execute(any(HttpGet.class))).then(
	...
	private static void assertWeatherDataList(
			List<StatusWeatherData> weatherDataList, int maxData) {
	...
	private static void assertForecastWeatherDataList(
			List<ForecastWeatherData> forecasts, int maxData) {
	...
	public void testPublicConstructorWithNullApiKey() {
		new OwmClient(null);
	}
		...
		when(mockHttpClient.execute(any(HttpGet.class))).then(
				new Answer<HttpResponse>() {
					...
					public HttpResponse answer(InvocationOnMock invocation)
						...
						assertEquals(TESTING_API_KEY, headers[0].getValue());

						HttpResponse response = new BasicHttpResponse(
								HttpVersion.HTTP_1_1, 200, "Ok");
						response.setEntity(new StringEntity(
								TestData.CURRENT_WEATHER_AROUND_CITY_COORD));
						return response;
					}
				});

		OwmClient owm = new OwmClient(TESTING_API_KEY, mockHttpClient);
		owm.currentWeatherAtCity(55f, 37f, 10);
	}

	@Test
	public void testCurrentWeatherAroundPoint() throws IOException,
			JSONException {
		HttpClient mockHttpClient = createHttpClientThatRespondsWith(TestData.CURRENT_WEATHER_AROUND_POINT);
		OwmClient owm = new OwmClient(TESTING_API_KEY, mockHttpClient);
		WeatherStatusResponse currentWeather = owm.currentWeatherAroundPoint(
				55f, 37f, 10);
		assertTrue(currentWeather.hasWeatherStatus());
		OwmClientTest.assertWeatherDataList(currentWeather.getWeatherStatus(),
				10);
	}

	@Test
	public void testCurrentWeatherAroundCity() throws IOException,
			JSONException {
		HttpClient mockHttpClient = createHttpClientThatRespondsWith(TestData.CURRENT_WEATHER_AROUND_CITY_COORD);
		OwmClient owm = new OwmClient(TESTING_API_KEY, mockHttpClient);
		WeatherStatusResponse currentWeather = owm.currentWeatherAtCity(55f,
				37f, 10);
		assertTrue(currentWeather.hasWeatherStatus());
		OwmClientTest.assertWeatherDataList(currentWeather.getWeatherStatus(),
				10);
	...
	@Test
	public void testCurrentWeatherInBoundingBox() throws IOException,
			JSONException {
		HttpClient mockHttpClient = createHttpClientThatRespondsWith(TestData.CURRENT_WEATHER_IN_BBOX);
		OwmClient owm = new OwmClient(TESTING_API_KEY, mockHttpClient);
		WeatherStatusResponse currentWeather = owm.currentWeatherInBoundingBox(
				12f, 32f, 15f, 37f);
		assertTrue(currentWeather.hasWeatherStatus());
		OwmClientTest.assertWeatherDataList(currentWeather.getWeatherStatus(),
				Integer.MAX_VALUE);
	...
	@Test
	public void testCurrentWeatherAtCityBoundingBox() throws IOException,
			JSONException {
		HttpClient mockHttpClient = createHttpClientThatRespondsWith(TestData.CURRENT_WEATHER_IN_CITY_BBOX);
		OwmClient owm = new OwmClient(TESTING_API_KEY, mockHttpClient);
		WeatherStatusResponse currentWeather = owm
				.currentWeatherAtCityBoundingBox(12f, 32f, 15f, 37f);
		assertTrue(currentWeather.hasWeatherStatus());
		OwmClientTest.assertWeatherDataList(currentWeather.getWeatherStatus(),
				Integer.MAX_VALUE);
	}
	...
	public void testCurrentWeatherInCircle() throws IOException, JSONException {
		...
		OwmClient owm = new OwmClient(TESTING_API_KEY, mockHttpClient);
		WeatherStatusResponse currentWeather = owm.currentWeatherInCircle(
				55.5f, 37.5f, 40f);
		assertTrue(currentWeather.hasWeatherStatus());
		OwmClientTest.assertWeatherDataList(currentWeather.getWeatherStatus(),
				Integer.MAX_VALUE);
	...
	@Test
	public void testCurrentWeatherAtCityInCircle() throws IOException,
			JSONException {
		HttpClient mockHttpClient = createHttpClientThatRespondsWith(TestData.CURRENT_WEATHER_IN_CIRCLE);
		OwmClient owm = new OwmClient(TESTING_API_KEY, mockHttpClient);
		WeatherStatusResponse currentWeather = owm.currentWeatherAtCityCircle(
				55.5f, 37.5f, 40f);
		assertTrue(currentWeather.hasWeatherStatus());
		OwmClientTest.assertWeatherDataList(currentWeather.getWeatherStatus(),
				Integer.MAX_VALUE);
	...
	@Test
	public void testCurrentWeatherAtStationId() throws IOException,
			JSONException {
	...
	@Test
	public void testCurrentWeatherAtCityNameWithCountryCode()
		...
		OwmClient owm = new OwmClient(TESTING_API_KEY, mockHttpClient);
		WeatherStatusResponse currentWeather = owm.currentWeatherAtCity(
				"london", "UK");
		assertTrue(currentWeather.hasWeatherStatus());
		OwmClientTest.assertWeatherDataList(currentWeather.getWeatherStatus(),
				Integer.MAX_VALUE);
	...
	public void testForecastAtCityId() throws IOException, JSONException {
		...
		OwmClient owm = new OwmClient(TESTING_API_KEY, mockHttpClient);
		WeatherForecastResponse forecastResponse = owm
				.forecastWeatherAtCity(524901);
		...
		OwmClientTest.assertForecastWeatherDataList(
				forecastResponse.getForecasts(), Integer.MAX_VALUE);
	}
	...
	@Test
	public void testHourHistoryWeatherAtCity() throws IOException,
			JSONException {
		HttpClient mockHttpClient = createHttpClientThatRespondsWith(TestData.HISTORY_WEATHER_AT_CITY_ID);
		OwmClient owm = new OwmClient(TESTING_API_KEY, mockHttpClient);
		WeatherHistoryCityResponse history = owm.historyWeatherAtCity(2885679,
				HistoryType.HOUR);
	...
	@Test
	public void testTickHistoryWeatherAtStation() throws IOException,
			JSONException {
		HttpClient mockHttpClient = createHttpClientThatRespondsWith(TestData.HISTORY_WEATHER_AT_STATION_ID_BY_TICK);
		OwmClient owm = new OwmClient(TESTING_API_KEY, mockHttpClient);
		WeatherHistoryStationResponse history = owm.historyWeatherAtStation(
				9040, HistoryType.TICK);
	...
	@Test
	public void testHourlyHistoryWeatherAtStation() throws IOException,
			JSONException {
		HttpClient mockHttpClient = createHttpClientThatRespondsWith(TestData.HISTORY_WEATHER_AT_STATION_ID_BY_HOUR);
		OwmClient owm = new OwmClient(TESTING_API_KEY, mockHttpClient);
		WeatherHistoryStationResponse historyResponse = owm
				.historyWeatherAtStation(9040, HistoryType.HOUR);
